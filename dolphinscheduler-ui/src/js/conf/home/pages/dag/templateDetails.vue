...
<template>
  <div class="home-main index-model">
    <m-dag v-if="!isLoading" :type="'template'" :release-state="releaseState"></m-dag>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      ...mapActions('dag', ['getProcessList','getProjectList', 'getResourcesList', 'getTemplateDetails','getResourcesListJar']),
      ...
      init () {
        ...
        Promise.all([
          // Node details
          this.getTemplateDetails(this.$route.params.id),
       ...
       */
      _reset () {
        this.getTemplateDetails(this.$route.params.id).then(res => {
          let item = res
          this.setIsDetails(item.releaseState === 'ONLINE')
          this.releaseState = item.releaseState
        })
