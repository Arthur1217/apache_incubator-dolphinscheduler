      ...
      <table class="fixed">
        <tr>
          ...
          <th scope="col" style="min-width: 200px;max-width: 300px;">
            <span>{{$t('Process Template Name')}}</span>
          </th>
          <th scope="col" style="min-width: 50px">
            <span>{{$t('State')}}</span>
          </th>
        ...
        <tr v-for="(item, $index) in list" :key="item.id">
          ...
          <td style="min-width: 200px;max-width: 300px;padding-right: 10px;">
            <span class="ellipsis">
              <router-link :to="{ path: '/projects/template/list/' + item.id}" tag="a" class="links" :title="item.name">
                {{item.name}}
              </router-link>
            </span>
          </td>
          ...
          <td style="z-index: inherit;">
            ...
            <x-button type="info" shape="circle" size="xsmall" data-toggle="tooltip" :title="$t('Copy Template')" @click="_copyTemplate(item)" :disabled="item.releaseState === 'ONLINE'" icon="ans-icon-copy"><!--{{$t('复制')}}--></x-button>
            <x-poptip
...
<script>
  ...
  export default {
    ...
    props: {
      processTemplateList: Array,
      pageNo: Number,
      pageSize: Number
    },
    methods: {
      ...mapActions('dag', ['editTemplateState', 'deleteTemplate', 'batchDeleteTemplate', 'exportTemplate', 'copyTemplate']),
      _rtPublishStatus (code) {
        return _.filter(publishStatus, v => v.code === code)[0].descp
      },
      /**
       ...
       */
      _edit (item) {
        this.$router.push({ path: `/projects/template/list/${item.id}` })
      },
      /**
       * Offline
       */
      _downline (item) {
        this.upTemplateState({
          processTemplateId: item.id,
          releaseState: 0
        })
      },
      ...
      /**
       * Create process based on template
       */
      _createProcess (item) {
        this.$router.push({
      ...
      _batchExport () {
        this.exportTemplate({
          processTemplateIds: this.strSelectIds,
          fileName: "process_template_"+new Date().getTime()
        }).then(res => {
          this._onUpdate()
          this.checkAll = false
          this.strSelectIds = ''
       ...
       */
      upTemplateState (o) {
        this.editTemplateState(o).then(res => {
          this.$message.success(res.msg)
          $('body').find('.tooltip.fade.top.in').remove()
          this._onUpdate()
        }).catch(e => {
